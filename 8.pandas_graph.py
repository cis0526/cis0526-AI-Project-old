...
from  mpl_finance import candlestick2_ohlc
  ...
  #matplot.finance 는 matplot 2.x버전 이후로 사용되지 않고,
  #mpl_finance로 사용된다고합니다.

import matplotlib.ticker as ticker

# 데이터 가져오기
engine = create_engine('mysql+pymysql://test:12345@13.125.126.9/data', encoding='utf8')
sql = 'SELECT * FROM USDT_BTC WHERE DATE BETWEEN \'2017-11-01\' and \'2017-11-02\' ;'
data = pd.read_sql(sql, engine)

data.head()

# 그래프 사이즈 지정 및 도화지 준비
fig = plt.figure(figsize=(len(data)/3, 5))
ax = fig.add_subplot(111)

# 캔들 차트의 x축 날짜데이터 생성하고 셋팅하기
x_pos = range(len(data))
x_name = []
for time in data.DATE:
    x_name.append(time.strftime('%H:%M'))

ax.xaxis.set_major_locator(ticker.FixedLocator(x_pos))
ax.xaxis.set_major_formatter(ticker.FixedFormatter(x_name))
fig.autofmt_xdate()

# 캔들차트 생성하기
mpl_finance.candlestick2_ohlc(ax, data['OPEN'], data['HIGH'], data['LOW'], data['CLOSE'], width=0.6, colorup='r', colordown='b')
...
candlestick2_ohlc(ax, data['OPEN'], data['HIGH'], data['LOW'], data['CLOSE'], width=0.6, colorup='r', colordown='b')
plt.grid()

# x축 상속, y축 동일하게 셋팅한 새로운 축을 생성하기
ax2 = ax.twinx()
ax2.set_ylim(ax.get_ylim())

# 9틱 이동평균 데이터를 생성하기
ma5 = data['CLOSE'].rolling(window = 9, min_periods = 1).mean()

# 녹색의 선 그래프로 그리기
ax2.plot(ma5, color='g', linewidth=1)
